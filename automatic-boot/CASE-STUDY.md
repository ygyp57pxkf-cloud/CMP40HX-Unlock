...
最初VDD同时提供120/144Hz，实际144Hz设置与NVENC探测通过后，后续复核曾回到120且物理屏拓扑发生变化，Sunshine日志期间没有新会话。无法据此确定哪个外部程序或显示事件触发了变化；最终将VDD收敛为仅1920×1080@144Hz，重载该虚拟设备后复核144Hz。客户端实流及下一次启动是否保持仍须验证。
