> **2026-09-14 更新：** 已新增并在本机部署 [Guard失败重试与Windows回退](guard/README.md)。原RefindPlus直接启动Shell遇到Device Error会停菜单；Guard改为返回后等30秒重试一次，再失败直接调用微软Windows。13种主机模拟场景和部署读回通过，新的真实固件启动/回退仍待验收；不保证恢复不返回的硬锁死。以下记录的成功路径是此前基线。
